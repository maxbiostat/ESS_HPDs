...
Alpha <- .95
symm_lcdf <- function(x) plnorm(q = x,
                              meanlog =  sym.Mu,
                              sdlog =  sqrt(sym.Sigmasq),
                              log.p = TRUE)
asymm_lcdf <- function(x) plnorm(q = x,
                               meanlog = asym.Mu,
...
pS <- function(k) prob_of_interest(logcdf = symm_lcdf, n = k,
                 a = sym.HPD[1],
                 b = sym.HPD[2])
pS <- Vectorize(pS)

pAS <- function(k) prob_of_interest(logcdf = asymm_lcdf, n = k,
                                   a = asym.HPD[1],
                                   b = asym.HPD[2])
pAS <- Vectorize(pAS)
...
curve(pS, 1, 500,
      ...
      main = "", lwd = 4, col = "blue")
curve(pAS, 1, 500, col = "gold", lwd = 4,  add = TRUE)
...
optimise(
  function(n) (pS(n)-IncProb)^2,
  c(1, 500)
)

optimise(
  function(n) (pAS(n)-IncProb)^2,
  c(1, 500)
)
